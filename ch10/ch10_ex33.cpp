...
https://github.com/cdude996/Cpp-Primer/blob/master/chapter10/10.33.cpp
...
void die(std::initializer_list<std::string> il) {
  for (const auto &arg : il) std::cerr << arg << " ";
  std::cerr << std::endl;
  exit(EXIT_FAILURE);
}

int main(int argc, char **argv) {
  if (argc != 4) die({"Usage:", *argv, "inFile oddfile evenfile"});

  std::ifstream inFile(*++argv);
  if (!inFile) die({"Could not open", *argv});
  ...
  if (in_iter == eof) die({"Invalid input"});
  return 0;
}