...
https://github.com/jaege/Cpp-Primer-5th-Exercises/blob/master/ch9/9.51.cpp
https://github.com/Mooophy/Cpp-Primer/blob/master/ch09/ex9_51.cpp
...
const std::vector<std::string> Date::months{"Jan", "Feb", "Mar", "Apr",
                                            "May", "Jun", "Jul", "Aug",
                                            "Sep", "Oct", "Nov", "Dec"};
...
Date::Date(const std::string &s) : Date() {
  if (s.find_first_of(alphabets) != std::string::npos) {
    ...
    for (std::vector<std::string>::size_type pos = 0; pos != months.size();
         ++pos) {
