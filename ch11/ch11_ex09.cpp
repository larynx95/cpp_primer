...
std::set<std::string> getwords(const std::string &ln) {
  ...
  while (start != ln.cend()) {
    start = find_if(start, ln.cend(), [](const char &c) { return isalpha(c); });
    end = find_if(start, ln.cend(), [](const char &c) { return !isalpha(c); });
    if (start != end) words_set.insert(std::string(start, end));
    start = end;
  }
  return words_set;
}
...
int main() {
  ...
  for (unsigned lineNumber = 1; getline(std::cin, line); ++lineNumber) {
    words = getwords(line);
    for (const auto word : words) word_position[word].push_back(lineNumber);
  }
  for (const auto &w : word_position) {
    std::cout << w.first << ": ";
    for (const auto &ln : w.second) std::cout << ln << ' ';
    std::cout << '\n';
  }
  return 0;
}
