...
multiset named bookstore from § 11.2.2 (p. 425). Write the variable’s type
...
int main() {
  ...
  // not using auto
  std::multiset<Sales_data, decltype(compareIsbn) *>::const_iterator iter_2 =
      bookstrore.cbegin();

  // using neither auto nor decltype
  std::multiset<Sales_data, bool (*)(const Sales_data &,
                                     const Sales_data &)>::const_iterator
      iter_3 = bookstrore.cbegin();
